...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda (user_id, movie_id, rating):((user_id, movie_id), rating))

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda (user_id, movie_id, rating):((user_id, movie_id), rating))
...
validationForPredictRDD = validationRDD.map(lambda (user_id, movie_id, rating) : (user_id, movie_id))
...
testForPredictingRDD = testRDD.map(lambda (uid, mid, rating): (uid, mid))
...
trainingAvgRating = trainingRDD.map(lambda (uid, mid, rating) : rating).reduce(lambda r1, r2 : (r1+r2))/trainingRDD.count()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda (uid, mid, rating) : (uid, mid, trainingAvgRating))
...
myUnratedMoviesRDD = (moviesRDD
                     ...
                     .filter(lambda (myUserID, mid): mid not in [mid for (myID, mid, rating) in myRatedMovies]))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (mid, (num, avg)): (mid, num))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda (myid, mid, rating) : (mid, rating))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                      .map(lambda (mid, ((PredictedRating, num), name)): (PredictedRating, name, num))
